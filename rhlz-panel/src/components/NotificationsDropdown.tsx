...
export default function NotificationsDropdown() {
  ...
  const [notifications, setNotifications] = useState<NotificationItem[]>(() => {
    try {
      const saved = localStorage.getItem("rhlz_notifications") || localStorage.getItem("raven_notifications");
  ...
  useEffect(() => {
    localStorage.setItem("rhlz_notifications", JSON.stringify(notifications));
    try { localStorage.removeItem("raven_notifications"); } catch {}
  }, [notifications]);

  // Live notices from the backend (RHLZ Keeper housekeeping, disk warnings...).
  useEffect(() => {
    const socket = io({ withCredentials: true });
    socket.on("keeper_notice", (n: any) => {
      if (!n?.title) return;
      const item: NotificationItem = {
        ...
        message: n.message || "",
        type: n.type || "info",
        timestamp: new Date(n.timestamp || Date.now()).toLocaleString(),
        read: false,
        link: n.link,
      };
