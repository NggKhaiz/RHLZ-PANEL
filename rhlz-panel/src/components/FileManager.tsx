...
export default function FileManager({ serverId }: { serverId: string }) {
  ...
  // Monotonic sequence so a slow response for an old path never overwrites
  // the listing of the current path (stale-fetch protection).
  ...
  const fetchFiles = async () => {
    ...
    try {
      ...
      if (seq !== fetchSeq.current) return; // stale response - a newer navigation is in flight
      if (res.data.isFile) {
