...
import { PRODUCT_NAME, PANEL_UI_NAME, VERSION } from "../brand";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const pName = panelName || PANEL_UI_NAME;
  const nameParts = pName.split(' ');
  const firstWord = nameParts[0]?.toUpperCase() || 'RHLZ';
  ...
  return (
    <div className={`flex h-[100dvh] w-full bg-transparent text-foreground font-sans overflow-hidden selection:bg-theme-600/30`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative bg-transparent">
        ...
        <footer className="flex items-center justify-between border-t border-line px-6 py-2.5 font-mono text-[10px] tracking-widest text-faint">
          <span>{PANEL_UI_NAME} v{VERSION}</span>
          <span>{PRODUCT_NAME} · © 2026</span>
        </footer>
      </div>
    </div>
  );
