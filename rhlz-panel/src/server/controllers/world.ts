...
import { resolveServerPath } from "../utils/paths.js";
...
export const getWorldInfo = async (req: Request, res: Response) => {
  try {
    ...
    const worldResolved = resolveServerPath(id, levelName);
    const worldDir = worldResolved.ok ? worldResolved.path : serverDir;
...
export const analyzeWorld = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const resolvedZip = resolveServerPath(id, zipPath);
    if (!resolvedZip.ok) {
      return res.status(403).json({ error: resolvedZip.error });
    }
    let zipFullPath = resolvedZip.path;
...
export const importWorld = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Never allow the destination to escape the server directory ("..", ".",
    // empty or absolute paths would otherwise delete/copy outside the root).
    const destResolved = resolveServerPath(id, chosenFolderName);
    if (!destResolved.ok || chosenFolderName === "." || chosenFolderName === ".." || chosenFolderName === "") {
      return res.status(400).json({ error: "Invalid world folder name" });
    }
    const finalWorldDestination = destResolved.path;
