...
import { extractArchive, assertSafeArchiveMembers } from "../utils/extract.js";
import { resolveServerPath, resolveBackupPath, relJoin } from "../utils/paths.js";
import { isDiskCriticallyFull } from "../services/keeper.js";
import { panelEvents } from "../events.js";
import { audit } from "../services/audit.js";
import { assertSafeDownloadUrl } from "../utils/ssrf.js";
import { scanForMinerSignatures } from "../utils/minerScan.js";
...
export const checkPort = async (req: Request, res: Response) => {
  ...
  const servers = await readJSON("servers.json") || [];
  const inUse = servers.some((s: any) => s.port === portNum);
  
  res.json({ inUse });
};

// Serializes power actions per server: concurrent start/stop/restart calls
// on the same runtime are rejected (409) instead of interleaving.
const powerBusy = new Set<string>();
...
export const createServer = async (req: Request, res: Response) => {
  ...
  if (await isDiskCriticallyFull()) {
    return res.status(503).json({ error: "Host storage is critically full. Server creation is temporarily blocked." });
  }
  ...
  const { name, ram, port, version, theme, cpu, disk, cpuLimit, diskLimit, owner, ownerId, ipAlias, type, nodeId, runtimeType, dockerImage, startupCommand } = req.body;
  if (!name || !ram || !port) {
  ...
  const cpuNum = (cpu ?? cpuLimit) === undefined ? 100 : Number(cpu ?? cpuLimit);
  const diskNum = (disk ?? diskLimit) === undefined ? 10 : Number(disk ?? diskLimit);
  if (!Number.isInteger(portNum) || portNum < 1 || portNum > 65535) {
    return res.status(400).json({ error: "Port must be an integer between 1 and 65535" });
  }
  if (!Number.isFinite(ramNum) || ramNum <= 0 || ramNum > 128) {
    return res.status(400).json({ error: "RAM must be a number between 1 and 128 GB" });
  }
  if (!Number.isFinite(cpuNum) || cpuNum <= 0 || cpuNum > 100000) {
    return res.status(400).json({ error: "CPU must be a positive number" });
  }
  if (!Number.isFinite(diskNum) || diskNum <= 0 || diskNum > 100000) {
    return res.status(400).json({ error: "Disk must be a positive number (GB)" });
  }
  ...
  // Strip control characters (CR/LF could inject extra server.properties
  // lines) and cap the length.
  ...
  if (servers.find((s: any) => s.port === portNum)) {
  ...
  try {
    ...
    await updateJSON("servers.json", (list: any[]) => list.map((s: any) => (s.id === id ? serverData : s)));
    await createSftpUser(id).catch(e => console.error("SFTP user creation failed:", e));
    await audit("server.create", { serverId: id, name: cleanName, by: user?.id });
    res.json(serverData);
  } catch (err: any) {
    console.error(err);
...
export const deleteServer = async (req: Request, res: Response) => {
  try {
    ...
    await audit("server.delete", { serverId: id, by: user?.id });
...
export const startServer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (powerBusy.has(id)) return res.status(409).json({ error: "Another power action is already in progress for this server" });
    powerBusy.add(id);
    const servers = await readJSON("servers.json") || [];
    ...
    try {
      ...
      if (!isGeneric) {
        ...
        if (!fs.existsSync(jarPath)) {
          ...
          panelEvents.emit("log", id, `[RHLZ] Pre-flight: server.jar not found. Downloading ${server.type} (${server.version || "latest"})...\r\n`);
          try {
            await downloadJar(server.type, server.version || "latest", jarPath);
            panelEvents.emit("log", id, `[RHLZ] server.jar downloaded successfully.\r\n`);
          } catch (dlErr: any) {
            panelEvents.emit("log", id, `[RHLZ] Notice: Automatic JAR download error: ${dlErr?.message || dlErr}\r\n`);
          }
        }
        const eulaPath = path.join(serverDir, "eula.txt");
        if (!fs.existsSync(eulaPath)) {
    ...
    await audit("server.start", { serverId: id, by: (req as any).user?.id });
...
export const stopServer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await writeJSON("servers.json", servers);
    await audit("server.stop", { serverId: id, by: (req as any).user?.id });
    res.json({ success: true });
  } catch (err: any) {
...
export const restartServer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await attachServerRuntimeSocket(server, server.id);
    await audit("server.restart", { serverId: id, by: (req as any).user?.id });
    res.json({ success: true, startedAt: server.startedAt });
  } catch (err: any) {
...
export const getFiles = async (req: Request, res: Response) => {
  ...
  const resolved = resolveServerPath(id, dirPath);
  if (!resolved.ok) {
...
export const uploadChunk = async (req: Request, res: Response) => {
  ...
  const resolved = resolveServerPath(id, relJoin(dirPath || "/", fileName + ".part"));
  if (!resolved.ok) {
    return res.status(403).json({ error: resolved.error });
  }
  const partFilePath = resolved.path;
  const targetPath = path.dirname(partFilePath);
  ...
  try {
    ...
    if (server.runtimeType !== "local" && server.containerId) {
      try {
        const status = await getServerRuntimeStatus(server);
        if (!status?.State?.Running) {
          panelEvents.emit("log", id, `[RHLZ] Refreshing Docker container environment...\r\n`);
          await deleteServerRuntime(server);
          server.containerId = await createServerRuntime(server);
          await writeJSON("servers.json", servers);
        }
    ...
    panelEvents.emit("log", id, `[RHLZ] Server JAR successfully installed and configured!\r\n`);
...
export const completeUpload = async (req: Request, res: Response) => {
  ...
  const resolvedFinal = resolveServerPath(id, relJoin(dirPath || "/", fileName));
  if (!resolvedFinal.ok) {
    return res.status(403).json({ error: resolvedFinal.error });
  }
  const finalFilePath = resolvedFinal.path;
...
export const uploadFile = async (req: Request, res: Response) => {
  ...
  if (req.file) {
    const dest = resolveServerPath(id, relJoin(dirPath, req.file.originalname));
    if (!dest.ok) {
      return res.status(403).json({ error: dest.error });
    }
    const destFile = dest.path;
...
export const deleteFile = async (req: Request, res: Response) => {
  ...
  try {
    for (const filePath of filePaths) {
      const resolved = resolveServerPath(id, filePath);
      if (!resolved.ok) {
        return res.status(403).json({ error: resolved.error });
      }
      await fs.remove(resolved.path);
    }
    res.json({ success: true });
  } catch (e: any) {
...
export const zipFiles = async (req: Request, res: Response) => {
  ...
  const resolvedBase = resolveServerPath(id, dirPath || "/");
  if (!resolvedBase.ok) {
    return res.status(403).json({ error: resolvedBase.error });
  }
  const baseDir = resolvedBase.path;

  const resolvedOut = resolveServerPath(id, relJoin(dirPath || "/", outputName || "archive.zip"));
  if (!resolvedOut.ok) {
    return res.status(403).json({ error: resolvedOut.error });
  }
  const outZipPath = resolvedOut.path;
  ...
  try {
    ...
    for (const name of fileNames) {
      const resolved = resolveServerPath(id, relJoin(dirPath || "/", name));
      if (!resolved.ok) {
        if (!res.headersSent) res.status(403).json({ error: resolved.error });
        return;
      }
      resolvedNames.push(resolved.path);
    }

    for (const filePath of resolvedNames) {
...
export const renameFile = async (req: Request, res: Response) => {
  ...
  const oldResolved = resolveServerPath(id, oldPath);
  const newResolved = resolveServerPath(id, newPath);
...
export const downloadFile = async (req: Request, res: Response) => {
  ...
  try {
    if (rawPaths.length === 1) {
      const resolved = resolveServerPath(id, rawPaths[0]);
      if (!resolved.ok) {
        return res.status(403).json({ error: resolved.error });
      }
      const targetPath = resolved.path;
    ...
    for (const relPath of rawPaths) {
      const resolved = resolveServerPath(id, relPath);
...
export const createFile = async (req: Request, res: Response) => {
  ...
  try {
    await fs.writeFile(resolved.path, "", "utf-8");
    res.json({ success: true });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
...
export const createBackup = async (req: Request, res: Response) => {
  ...
  try {
    ...
    output.on("error", (err: any) => {
      console.error("Backup output error:", err);
      if (!res.headersSent) res.status(500).json({ error: err.message });
    });
    output.on("close", () => {
...
export const downloadBackup = async (req: Request, res: Response) => {
  ...
  const resolved = resolveBackupPath(id, filename);
  if (!resolved.ok) {
...
export const installPlugin = async (req: Request, res: Response) => {
  ...
  if (req.body.downloadUrl) {
     try {
        const url = String(req.body.downloadUrl);
        ...
        const filename = String(req.body.filename || "plugin.jar");
        const dest = resolveServerPath(id, relJoin("plugins", filename));
        if (!dest.ok) {
          return res.status(403).json({ error: dest.error });
        }
        ...
        if (url === 'dummy') {
          ...
          const response = await axios({ url, method: 'GET', responseType: 'stream', maxContentLength: 200 * 1024 * 1024, timeout: 60000 });
          const writer = fs.createWriteStream(filePath);
          response.data.pipe(writer);
          await new Promise<void>((resolve, reject) => { writer.on('finish', resolve); writer.on('error', reject); });
        }
        const scan = await scanForMinerSignatures(filePath);
        if (scan.blocked) {
          await fs.remove(filePath).catch(() => {});
          void audit("plugin.blocked_miner", { serverId: id, label: scan.label, by: (req as any).user?.id });
          return res.status(403).json({ error: "Downloaded artifact contains cryptocurrency-mining signatures and was blocked." });
        }
  ...
  try {
    ...
    const commonHeaders = {
      'User-Agent': 'RHLZPanel/1.0.0 (https://github.com/NggKhaiz/RHLZ-PANEL)'
    };
    ...
    const filePath = dest.path;
    const ssrf = await assertSafeDownloadUrl(String(downloadUrl));
    if (!ssrf.ok) {
      return res.status(400).json({ error: ssrf.error });
    }
...
export const installMod = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const dest = resolveServerPath(id, relJoin("mods", filename));
    if (!dest.ok) {
      return res.status(403).json({ error: dest.error });
    }
    const filePath = dest.path;
    ...
    if (scan.blocked) {
      await fs.remove(filePath).catch(() => {});
      void audit("mod.blocked_miner", { serverId: id, label: scan.label, by: (req as any).user?.id });
      return res.status(403).json({ error: "Downloaded artifact contains cryptocurrency-mining signatures and was blocked." });
    }
...
export const updateResources = async (req: Request, res: Response) => {
  try {
    ...
    if (!Number.isFinite(ramNum) || ramNum <= 0 || ramNum > 128 ||
        !Number.isFinite(cpuNum) || cpuNum <= 0 || cpuNum > 100000 ||
        !Number.isFinite(diskNum) || diskNum <= 0 || diskNum > 100000) {
      return res.status(400).json({ error: "Resources must be positive numbers within allowed ranges" });
    }
    const servers = await readJSON("servers.json") || [];
    const server = servers.find((s: any) => s.id === id);
    if (!server) return res.status(404).json({ error: "Server not found" });
...
export const updateSuspend = async (req: Request, res: Response) => {
  try {
    ...
    const ALLOWED = ["permanent", "1_month", "2_months", "24_hours", "1_week"];
    if (suspendDuration !== null && (typeof suspendDuration !== "string" || !ALLOWED.includes(suspendDuration))) {
      return res.status(400).json({ error: "Invalid suspend duration" });
    }
    const servers = await readJSON("servers.json") || [];
    const server = servers.find((s: any) => s.id === id);
    if (!server) return res.status(404).json({ error: "Server not found" });
    ...
    await writeJSON("servers.json", servers);
    await audit("server.suspend", { serverId: id, suspended: server.suspended, by: (req as any).user?.id });
...
export const restoreBackup = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // RHLZ: reject backups whose members could escape the server dir
    // (extract-zip has an unfixed symlink-traversal advisory; we pre-scan).
    await assertSafeArchiveMembers(backupPath);
