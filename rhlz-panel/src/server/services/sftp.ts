...
// RHLZ: brute-force + resource caps for the SFTP service.
...
function clientIp(client: any): string {
  return client?._sock?.remoteAddress || client?.remoteAddress || "unknown";
}

function isBanned(ip: string): boolean {
  const cur = authFailures.get(ip);
  if (!cur) return false;
  // until === 0 means "counting failures, not banned yet" - keep the entry.
  if (cur.until > 0 && cur.until < Date.now()) {
...
function recordFailure(ip: string): boolean {
  const cur = authFailures.get(ip) || { count: 0, until: 0 };
  const count = cur.count + 1;
  if (count >= MAX_AUTH_FAILURES) {
    authFailures.set(ip, { count, until: Date.now() + BAN_MS });
    return true; // now banned
  }
  authFailures.set(ip, { count, until: 0 });
  return false;
}
...
export async function initSFTPServer() {
  ...
  const server = new Server({ hostKeys: [hostKey] }, (client) => {
    ...
    // Connection cap: refuse when too many concurrent sessions.
    ...
    client.on("authentication", async (ctx) => {
      try {
        if (isBanned(ip)) {
          // Reject without any further work; the ban is enforced at the door.
        ...
        if (match) {
          authFailures.delete(ip); // success clears the failure state
          sftpUser = user;
          ctx.accept();
        } else {
          await new Promise((r) => setTimeout(r, 500));
