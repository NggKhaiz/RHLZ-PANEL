...
/** Workload types with Minecraft/proxy semantics; everything else is a generic
 *  application runtime (node, python, go, rust, c/c++, c#, ruby, php, bash...). */
export const MC_SOFTWARE = new Set([
...
export function isMinecraftSoftware(type: string): boolean {
  return MC_SOFTWARE.has(String(type || "").toUpperCase());
}
