...
const PIDS_DIR = () => path.join(process.cwd(), ".data", "pids");

// Workload types with built-in Minecraft/proxy semantics; everything else is a
// generic application runtime driven by its startup command.
const KNOWN_MC = new Set([
  "paper", "spigot", "bukkit", "craftbukkit", "purpur", "folia", "vanilla",
  "forge", "fabric", "neoforge", "quilt", "mohist", "arclight", "custom",
  "velocity", "bungeecord", "waterfall", "bungee",
]);

async function trackPid(id: string, pid: number): Promise<void> {
  try {
    await fs.ensureDir(PIDS_DIR());
    await fs.writeFile(path.join(PIDS_DIR(), `${id}.pid`), String(pid), "utf8");
  } catch (e) {}
}

async function untrackPid(id: string): Promise<void> {
  try {
    await fs.remove(path.join(PIDS_DIR(), `${id}.pid`));
  } catch (e) {}
}

/**
 * Kills child processes left behind by a previous panel instance (crash,
 * OOM kill, manual restart). Only touches processes whose working directory
 * is inside .data/servers/, i.e. processes this panel actually spawned -
 * a reused PID for an unrelated process is never killed.
 */
export async function reapOrphanedLocalProcesses(): Promise<string[]> {
  const reaped: string[] = [];
  try {
    const dir = PIDS_DIR();
    if (!(await fs.pathExists(dir))) return reaped;
    ...
    for (const file of files) {
      ...
      try {
        ...
        const serverBase = path.resolve(process.cwd(), ".data", "servers");
        if (cwd.startsWith(serverBase)) {
          try { process.kill(pid, "SIGTERM"); } catch (e) {}
          // Give it a moment, then force-kill if still alive.
          setTimeout(() => {
            try { process.kill(pid, "SIGKILL"); } catch (e) {}
          }, 1500);
          reaped.push(file.replace(/\.pid$/, ""));
        }
      } catch (e) {}
...
export const createLocalServer = async (serverData: any) => {
  ...
  } else if (!KNOWN_MC.has(type)) {
    // Generic language runtime (go, rust, c/c++, c#, ruby, php, bash, ...):
    // nothing to seed; the startup command drives it.
  } else {
...
export const startLocalServer = async (id: string, serverData: any) => {
  ...
  if (type === "nodejs" || type === "node") {
    ...
    // Generic language runtime: run the configured startup command directly.
    if (serverData.startupCommand && serverData.startupCommand.trim()) {
      const parts = serverData.startupCommand.trim().split(/\s+/);
      const bin = parts[0];
      const args = parts.slice(1);
      ...
      logMessage(`Generic runtime "${type}" requires a startup command. Set one in Server Settings.`);
      throw new Error(`Generic runtime "${type}" requires a startup command`);
    }
  } else {
    const jarPath = path.join(serverPath, "server.jar");
  ...
  child.on("spawn", () => {
    ...
    if (child.pid) trackPid(id, child.pid);
