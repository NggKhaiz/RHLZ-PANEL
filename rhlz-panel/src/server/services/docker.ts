...
export const getVersions = async (type: string = "PAPER") => {
  ...
  const GENERIC = new Set(["GO", "RUST", "CPP", "C", "CSHARP", "DOTNET", "RUBY", "PHP", "BASH", "SH", "JAVA", "GENERIC"]);
  if (GENERIC.has(normalizedType)) {
    return ["latest"];
  }
...
export const createServerContainer = async (serverData: any, nodeId?: string) => {
  ...
  const MC_TYPES = new Set(["PAPER", "SPIGOT", "BUKKIT", "CRAFTBUKKIT", "PURPUR", "FOLIA", "VANILLA", "FORGE", "FABRIC", "NEOFORGE", "QUILT", "MOHIST", "ARCLIGHT", "CUSTOM"]);
  const PROXY_TYPES = new Set(["VELOCITY", "BUNGEECORD", "WATERFALL", "BUNGEE"]);
  const isProxy = PROXY_TYPES.has(serverType);
  const isMc = MC_TYPES.has(serverType);
  // Generic application runtimes: node, python, go, rust, c/c++, c#, ruby,
  // php, bash, ... anything that is not a Minecraft or proxy workload.
  const isGenericApp = !isMc && !isProxy;
  const isNode = serverType === "NODEJS" || serverType === "NODE";
  const isPython = serverType === "PYTHON" || serverType === "PYTHON3";
  ...
  if (isNode) {
    ...
    if (!serverData.dockerImage) {
      throw new Error(`No docker image configured for generic runtime "${serverType}". Set a docker image or a supported runtime type.`);
    }
    shortImage = serverData.dockerImage;
    fullImage = serverData.dockerImage;
  }
  
  if (isMc && serverData.dockerImage) {
    shortImage = serverData.dockerImage;
    fullImage = serverData.dockerImage;
  }
  ...
  if (isNode) {
    ...
    // Generic language runtimes (go, rust, c/c++, c#, ruby, php, bash, ...)
    envVars = [
      `PORT=${serverData.port}`,
      `SERVER_PORT=${serverData.port}`,
  ...
  const buildContainerOptions = (img: string) => {
    ...
    if (isGenericApp && !isNode && !isPython) {
      cmd = ["/bin/sh", "-c", serverData.startupCommand || "echo 'No startup command configured for this runtime.'; sleep infinity"];
    }
    ...
    return {
      Image: img,
      name: `rhlz-server-${serverData.id}`,
      Tty: true,
      OpenStdin: true,
      StdinOnce: false,
      Env: envVars,
      ...
      HostConfig: {
        ...
        Binds: binds,
        // Enforce hard resource caps from the server's allocation so a single
        // game server can never exhaust the host (memory + CPU pinning).
        Memory: Math.round((serverData.ram || 2) * 1024 * 1024 * 1024),
        MemorySwap: Math.round((serverData.ram || 2) * 1024 * 1024 * 1024), // no swap headroom
        NanoCpus: Math.round((serverData.cpu || 100) * 1e7)
      }
    };
  };
  ...
  try {
    ...
    if (err?.statusCode === 409 || errStr.includes("409") || errStr.includes("Conflict") || errStr.includes("already in use")) {
      console.log(`Container name collision for rhlz-server-${serverData.id}. Removing stale container...`);
      try {
        const oldCont = docker.getContainer(`rhlz-server-${serverData.id}`);
        ...
        if (["NODEJS", "NODE"].includes(type)) {
          ...
          if (!fs.existsSync(pkgPath)) {
            await fs.writeFile(pkgPath, JSON.stringify({
              ...
              version: "1.0.0",
              description: "Node.js app on RHLZ Panel",
              main: "index.js",
  ...
  try {
    ...
    if (server) {
      ...
      if (!isGeneric) {
        const jarPath = path.join(serverDir, "server.jar");
        if (!fs.existsSync(jarPath)) {
          panelEvents.emit("log", server.id, `[RHLZ] Downloading ${server.type} (${server.version || "latest"}) server JAR...\r\n`);
          try {
            await downloadJar(server.type, server.version || "latest", jarPath);
            panelEvents.emit("log", server.id, `[RHLZ] Server JAR downloaded successfully.\r\n`);
          } catch (err: any) {
            panelEvents.emit("log", server.id, `[RHLZ] Warning: JAR download error: ${err?.message || err}\r\n`);
          }
        }
        const eulaPath = path.join(serverDir, "eula.txt");
        if (!fs.existsSync(eulaPath)) {
...
export const deleteContainer = async (containerId: string, nodeId?: string) => {
  ...
  try {
    ...
    // Release the attached console stream so its file descriptor does not leak.
    const stream = activeStreams[containerId];
    if (stream) {
