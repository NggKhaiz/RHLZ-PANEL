...
import { requireServerAccess } from "../middleware/serverAccess.js";
...
const upload = multer({
  ...
  limits: { fileSize: 2 * 1024 * 1024 * 1024, files: 1 } // 2 GB per chunk/file
});
...
// Every /:id-scoped route below requires the caller to have access to that server.
...
router.get("/:id/subusers", async (req, res) => {
  ...
  const serverRec = (req as any).server;
  if (reqUser.role !== "admin" && reqUser.role !== "owner" && serverRec?.owner !== reqUser.id) {
    return res.status(403).json({ error: "Forbidden: only admins or the server owner can manage sub-users" });
  }
...
router.post("/:id/sftp/create", async (req, res) => {
  ...
  const serverRec = (req as any).server;
  if (reqUser.role !== "admin" && reqUser.role !== "owner" && serverRec?.owner !== reqUser.id) {
    return res.status(403).json({ error: "Forbidden: only admins or the server owner can manage SFTP credentials" });
  }
