...
import { PANEL_UI_NAME } from "../../brand.js";
...
router.post("/users", async (req, res) => {
  ...
  void audit("admin.user.create", { username: username.trim(), role: targetRole, by: user?.id });
...
router.put("/users/:id/role", async (req, res) => {
  ...
  await writeJSON("users.json", users);
  void audit("admin.user.role", { userId: users[targetIndex].id, role: targetRole, by: user?.id });
...
router.delete("/users/:id", async (req, res) => {
  ...
  await writeJSON("users.json", users);
  void audit("admin.user.delete", { userId: req.params.id, by: user?.id });
...
router.put("/users/:id/password", async (req, res) => {
  ...
  await writeJSON("users.json", users);
  void audit("admin.user.password", { userId: users[targetIndex].id, by: user?.id });
  res.json({ success: true });
});
...
router.put("/settings", async (req, res) => {
  ...
  if (panelName !== undefined) {
    settings.panelName = panelName || PANEL_UI_NAME;
    try {
  ...
  if (keeper !== undefined && typeof keeper === "object" && keeper !== null) {
    settings.keeper = { ...(settings.keeper || {}), ...keeper };
  }

  if (schedules !== undefined && Array.isArray(schedules)) {
    // Sanitize: only known actions and valid HH:MM times are kept.
    settings.schedules = schedules
      .filter((s: any) => s && ["start", "stop", "restart"].includes(s.action) && typeof s.time === "string")
      .map((s: any, i: number) => ({
        id: typeof s.id === "string" ? s.id : "sched-" + i,
  ...
  void audit("admin.settings", { by: user?.id });
