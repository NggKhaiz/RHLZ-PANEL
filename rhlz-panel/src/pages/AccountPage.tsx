...
export default function AccountPage(): React.ReactElement {
  ...
  const verifyTwoFactor = async () => {
    setIsTotpBusy(true);
    try {
      await axios.post("/api/auth/2fa/verify", { code: totpCode });
  ...
  const disableTwoFactor = async () => {
    setIsTotpBusy(true);
    try {
      await axios.post("/api/auth/2fa/disable", { code: totpCode });
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-7xl space-y-8 pb-12 px-4 md:px-0 mt-8 w-full">
        ...
        {/* Two-Factor Authentication (TOTP) */}
        <section className="relative z-10 mt-8 rounded-2xl border border-border-subtle bg-card p-6 md:p-8 shadow-xl">
          <div className="flex items-start justify-between gap-4 mb-2">
            <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
              <Shield className="text-theme-500 h-5 w-5" /> Two-Factor Authentication
            </h3>
            <span className={`text-[10px] font-mono uppercase tracking-wider px-2.5 py-1 rounded-full border ${
              user.totpEnabled ? "bg-theme-600/10 text-theme-500 border-theme-600/30" : "bg-muted text-muted-foreground border-border"
            }`}>
              {user.totpEnabled ? "Enabled" : "Disabled"}
            </span>
          </div>
          <p className="text-xs text-muted-foreground mb-5 max-w-lg leading-relaxed">
            Add a one-time-password step to sign-in using any TOTP app (Google Authenticator, Authy, 1Password...). Recovery codes are shown once — store them somewhere safe.
          </p>
          ...
          {!user.totpEnabled ? (
            totpSetup ? (
              <div className="flex flex-col gap-5 max-w-lg">
                <div className="flex flex-col sm:flex-row gap-5 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <code className="block font-mono text-xs bg-black/40 border border-border rounded-lg px-3 py-2 break-all text-theme-300">
                      {totpSetup.secret}
                    </code>
                    <p className="text-xs text-muted-foreground mt-3 mb-1">Recovery codes (one-time use, shown once)</p>
                    <div className="grid grid-cols-2 gap-1 font-mono text-[11px] text-foreground-muted">
                      {totpSetup.recoveryCodes.map((c: string) => <span key={c}>{c}</span>)}
                    </div>
                  </div>
                </div>
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-muted border border-border focus:border-theme-600 focus:ring-1 focus:ring-theme-600/50 rounded-xl px-4 py-2.5 text-foreground outline-none"
                  />
                  <button
                    onClick={verifyTwoFactor}
                    disabled={isTotpBusy || totpCode.length !== 6}
                    className="bg-theme-600 hover:bg-theme-700 disabled:opacity-50 text-foreground font-semibold px-5 py-2.5 rounded-xl transition-all whitespace-nowrap"
                  >
                    {isTotpBusy ? "Verifying..." : "Verify & Enable"}
                  </button>
                </div>
              </div>
            ) : (
              <button
                ...
                disabled={isTotpBusy}
                className="bg-theme-600 hover:bg-theme-700 disabled:opacity-50 text-foreground font-semibold px-6 py-2.5 rounded-xl transition-all shadow-[0_0_15px_rgba(var(--theme-rgb-600),0.25)]"
              >
                {isTotpBusy ? "Generating..." : "Enable Two-Factor Authentication"}
              </button>
            )
          ) : (
            <div className="flex gap-3 max-w-md">
              ...
              <button
                ...
                disabled={isTotpBusy || totpCode.length !== 6}
                className="bg-muted hover:bg-muted-hover disabled:opacity-50 text-foreground font-semibold px-5 py-2.5 rounded-xl border border-border transition-all whitespace-nowrap"
              >
                {isTotpBusy ? "Disabling..." : "Disable 2FA"}
              </button>
            </div>
          )}
        </section>

      {(isChangingUsername || isChangingPassword || isTotpBusy) && <LoadingOverlay />}
    </motion.div>
  );
}