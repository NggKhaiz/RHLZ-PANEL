...
import { PANEL_UI_NAME, VERSION, COPYRIGHT } from "../brand";
...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.twoFactorRequired) {
        setTwoFactor(true);
        setTwoFactorToken(res.data.tempToken);
        setError("");
        return;
      }
  ...
  const handleTwoFactorLogin = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await axios.post("/api/auth/2fa/login", {
      ...
      setError(err.response?.data?.error || "Two-factor verification failed");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="desert-wrapper" onMouseMove={handleMouseMove}>
      ...
      <div className="login-card">
        ...
        {twoFactor ? (
          <form onSubmit={handleTwoFactorLogin} className="login-form">
            ...
            <button
              ...
              style={{ background: "rgba(255,255,255,0.08)", marginTop: "0.5rem" }}
              onClick={() => { setTwoFactor(false); setTwoFactorToken(""); setTwoFactorCode(""); setError(""); }}
            >
              Back
            </button>
          </form>
      ...
      <p style={{ marginTop: "2rem", textAlign: "center", fontSize: "0.72rem", color: "rgba(255, 255, 255, 0.35)", fontFamily: "monospace" }}>
        {PANEL_UI_NAME} v{VERSION} · {COPYRIGHT}
      </p>
